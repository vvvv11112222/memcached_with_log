...
static void drive_machine(conn *c)
{
    ...
    while (!stop)
    {
        ...
        case conn_listening:
            // 1. 开始连接：准备接受客户端连接请求
            LOGMC_INFO("开始监听客户端连接 [监听套接字: %d, 端口: %d]", c->sfd, settings.port);
            ...
            if (sfd == -1)
            {
                ...
                {
                    use_accept4 = 0;
                    LOGMC_ERROR("系统不支持accept4，自动切换为accept");
                    continue;
                }
                ...
                {
                    // 临时错误（非阻塞模式下无连接请求）
                    LOGMC_DEBUG("暂时无客户端连接请求 [错误: %s]", strerror(errno));
                    stop = true;
                }
                else if (errno == EMFILE)
                {
                    // 连接数超限
                    LOGMC_ERROR("连接失败：达到最大连接数限制 [当前限制: %d]", settings.maxconns);
                    if (settings.verbose > 0)
                ...
                {
                    // 其他错误（如系统调用失败）
                    LOGMC_ERROR("连接接受失败 [函数: %s, 错误: %s]",
                                use_accept4 ? "accept4()" : "accept()", strerror(errno));
            ...
            LOGMC_INFO("连接成功 [客户端IP: %s, 客户端端口: %d, 新套接字: %d]",
                       client_ip, client_port, sfd);
            ...
            if (!use_accept4)
            {
                ...
                {
                    LOGMC_ERROR("设置套接字非阻塞模式失败 [套接字: %d, 错误: %s]",
                                sfd, strerror(errno));
            ...
            if (settings.maxconns_fast)
            {
                ...
                if (reject)
                {
                    ...
                    STATS_UNLOCK();
                    LOGMC_ERROR("连接被拒绝：超过最大连接数 [客户端IP: %s, 限制: %d]",
                                client_ip, settings.maxconns);
                }
            }
            else
            {
        ...
        case conn_parse_cmd:
            ...
            do
            {
                size_t _len = strcspn((char *)c->rcurr, "\r\n");
                size_t _cap = _len < 1024 ? _len : 1024;
                char _line[1025];
                memcpy(_line, c->rcurr, _cap);
                _line[_cap] = '\0';
                LOGMC_INFO(">>> [fd=%d] 收到命令行: %s", c->sfd, _line);
            } while (0);
        ...
        case conn_closing:
            ...
            {
                LOGMC_DEBUG("关闭TCP套接字");
                conn_close(c); // 内部会调用close(sfd)释放套接字
                LOGMC_INFO("TCP连接已断开");
            }
        ...
        case conn_closed:
            // 修复：同样用空语句分隔标签和变量声明
            ; // 关键：添加分号作为空语句
            const char *closed_ip = (c->client_ip[0] != '\0') ? c->client_ip : "unknown";
            int closed_port = (c->client_port > 0) ? c->client_port : -1;

            LOGMC_ERROR("意外进入已关闭状态 [客户端IP: %s, 客户端端口: %d]，触发程序终止",
                        closed_ip, closed_port);
            abort();
            break;
...
{
    const char *log_path = "./memcached.log"; // 日志文件路径（可通过配置参数自定义）
    if (logmc_init(log_path, LOGMC_LEVEL_INFO) != 0)
    {
        fprintf(stderr, "警告：日志模块初始化失败，将使用默认输出\n");
    }
    else
    {
        LOGMC_INFO("Memcached 服务启动 [版本: %s, 端口: %d]", VERSION, settings.port);
    }
    int c;
